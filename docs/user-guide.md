...
- SQLite persistence for runs, chat history, learning excerpts, and repository-scoped knowledge
...
The optional top-level `repo_name` field scopes repository knowledge. When it is set, MergeMate loads repository-specific knowledge snippets for that repository and includes them in later prompts. Each run also stores the repository scope that was active at submission time, so resumed or later-inspected runs keep their original repo context even if the config changes. If you run the bot against more than one repository, keep `repo_name` aligned with the active repository in each config file.
...
If you need to reattach directly to a still-running session, use `mergemate resume --session ...`. The command scans the session's full run history, prints the incomplete run summary and recent conversation history, auto-approves the run when it is waiting for confirmation, and then waits for the original run to reach a terminal state. If there is no active run left to resume, it still prints the latest run summary and recent conversation history before exiting with a nonzero status.
...
mergemate search-runs "rate limit" --session my-feature

mergemate search-conversations "docker"
mergemate search-conversations "status update" --limit 5
mergemate search-conversations "rate limit" --session my-feature
```
