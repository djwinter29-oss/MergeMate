...
The optional top-level `repo_name` field scopes repository knowledge. When it is set, MergeMate loads repository-specific knowledge snippets for that repository and includes them in later prompts. If you run the bot against more than one repository, keep `repo_name` aligned with the active repository in each config file.
