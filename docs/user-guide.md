...
If you need to reattach directly to a still-running session, use `mergemate resume --session ...`. The command scans the session's full run history, prints the incomplete run summary and recent conversation history, and then waits for the original run to reach a terminal state. If there is no active run left to resume, it still prints the recent conversation history before exiting with a nonzero status.
